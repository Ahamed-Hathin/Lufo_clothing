...
const AccessoriesPage = ({ searchTerm = "" }) => {
    ...
    const fetchProducts = async () => {
        try {
            setLoading(true);
            const response = await axios.get('/product/accessories');
    ...
    if (loading) return <div className="text-center py-5"><h4>Loading Accessories...</h4></div>;
    ...
    return (
        <div>
            <div className="text-white text-center py-5 mb-5 rounded-4 shadow-sm"
                ...
                <h1 className="fw-bolder display-4 mb-2 text-uppercase">Exclusive Accessories</h1>
                <p className="lead opacity-75">Premium details for an elegant look</p>
            </div>
            ...
            <div className="row">
                {filtered.length > 0 ? filtered.map(product => (
                    ...
                    <div className="text-center py-5">
                        <p className="lead">No accessories found.</p>
                    </div>
                )}
            </div>
        </div>
