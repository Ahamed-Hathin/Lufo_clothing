...
const WomenPage = ({ searchTerm = "" }) => {
    ...
    const fetchWomenProducts = async () => {
        try {
            setLoading(true);
            const response = await axios.get('/product/women');
    ...
    return (
        <div>
            <div className="text-white text-center py-5 mb-5 rounded-4 shadow-sm"
                style={{ background: 'linear-gradient(135deg, #be185d 0%, #db2777 100%)' }}>
                <h1 className="fw-bolder display-4 mb-2">Women's Boutique</h1>
                <p className="lead opacity-75">Elegant styles for every occasion</p>
            </div>
            ...
            <div className="row">
                {filtered.length > 0 ? filtered.map(product => (
                    ...
                    <div className="text-center py-5">
                        <p className="lead">No products found for Women.</p>
                    </div>
                )}
            </div>
        </div>
